...
export default function FinalReport() {
  const { inspections, currentUser, getReportsForInspection, addReport, updateReport, completeReport, requestApproval, approveReport, addReportVersion, getReportVersions } = useApp();
  ...
  const eligibleInspections = useMemo(() =>
    inspections.filter(i => {
      ...
      return idx >= statusOrder.indexOf("1차 점검완료") || i.first_inspection_done_date;
    }),
    [inspections]
  );
  ...
  const existingReports = useMemo(() =>
    selectedInspectionId ? getReportsForInspection(selectedInspectionId, "final") : [],
  ...
  // Get first report's serial numbers for auto-fill
  ...
  return (
    <div className="pb-20 space-y-4">
      <h1 className="text-xl font-semibold">완료 점검보고서</h1>
          ...
          <Select value={selectedInspectionId} onValueChange={setSelectedInspectionId}>
            ...
            <SelectContent className="bg-popover z-[60]">
              ...
              {eligibleInspections.length === 0 && (
                <div className="px-3 py-2 text-xs text-muted-foreground">1차 점검완료 이상인 건이 없습니다.</div>
              )}
            </SelectContent>
          </Select>
        </CardContent>
        ...
        <>
          {activeReport ? (
            <ReportView
              ...
              onComplete={() => { completeReport(activeReport.id); toast.success("완료 점검보고서가 완료되었습니다."); }}
      ...
      {selectedInspection && (
        <Dialog open={reportModalOpen} onOpenChange={setReportModalOpen}>
          <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>완료 점검보고서 작성</DialogTitle>
            </DialogHeader>
            <CreateFinalReportForm
              inspection={selectedInspection}
              firstReportSerials={firstReports.length > 0 ? firstReports[firstReports.length - 1].serial_numbers : {}}
              onSubmit={(data) => {
                addReport(data);
...
function CreateFinalReportForm({
  ...
  firstReportSerials: Record<string, string>;
  onSubmit: (data: Omit<InspectionReport, "id" | "created_at" | "updated_at" | "completed_at" | "approved_at" | "approved_by">) => void;
  inspectorName: string;
}) {
  // Auto-fill serial numbers from 1차 report or equipment items
  const [serialNumbers] = useState<Record<string, string>>(() => {
    const map: Record<string, string> = {};
    inspection.equipment_items.forEach(item => {
      map[item.id] = firstReportSerials[item.id] || item.serial_no || "";
    });
    return map;
  });
  ...
  return (
    <div className="space-y-5">
      <div>
        <h3 className="text-sm font-semibold mb-2 text-primary">기본정보</h3>
      ...
      <div>
        <h3 className="text-sm font-semibold mb-2 text-primary">장비 및 Serial No (1차 보고서 연동)</h3>
        <Table>
          <TableHeader>
            <TableRow>
            ...
            {inspection.equipment_items.map(item => (
              <TableRow key={item.id}>
                ...
                <TableCell className="text-xs">
                  <Input className="h-8 text-xs bg-muted" value={serialNumbers[item.id] || "—"} disabled />
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
      ...
      <div>
        <h3 className="text-sm font-semibold mb-2 text-primary">최종 점검 결과</h3>
        <div className="space-y-3">
          <div>
            <label className="text-xs text-muted-foreground">점검 결과</label>
            <Textarea className="text-xs" rows={4} value={inspectionResult} onChange={e => setInspectionResult(e.target.value)} placeholder="최종 점검 결과를 입력하세요..." />
          </div>
          <div>
      ...
      <Button onClick={() => onSubmit({
        ...
        created_date: today,
      })} className="w-full">임시저장</Button>
    </div>
  );
}

/* ─── Report View (shared with FirstReport) ─── */
function ReportView({
  inspection, report, canEdit, canApprove,
  onUpdate, onComplete, onRequestApproval, onApprove,
  onAddVersion, getVersions, currentUserName, reportTitle,
}: {
  inspection: OutboundInspection;
  ...
  const handleSave = () => {
    onUpdate(report.id, { inspection_result: inspectionResult, special_notes: specialNotes });
    toast.success("임시저장되었습니다.");
  };

  const handleWordDownload = async () => {
    const updatedReport = { ...report, inspection_result: inspectionResult, special_notes: specialNotes };
  ...
  const statusColor: Record<string, string> = { draft: "bg-muted text-muted-foreground", completed: "bg-primary/15 text-primary", approval_requested: "bg-accent/15 text-accent", approved: "bg-primary/20 text-primary" };
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            ...
            <Badge className={cn("text-[10px]", statusColor[report.status])}>{statusLabel[report.status]}</Badge>
          </div>
        </CardHeader>
      ...
      {/* Equipment with locked serial nos */}
      <Card>
        <CardHeader className="pb-2"><CardTitle className="text-sm text-primary">장비 / Serial No</CardTitle></CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {inspection.equipment_items.map(item => (
                <TableRow key={item.id}>
                  <TableCell className="text-xs">{item.equipment_name}</TableCell>
                  <TableCell className="text-xs">{item.qty_set}</TableCell>
                  <TableCell className="text-xs">{report.serial_numbers[item.id] || item.serial_no || "—"}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
      ...
      <Card>
        <CardHeader className="pb-2"><CardTitle className="text-sm text-primary">점검 결과</CardTitle></CardHeader>
      ...
      <Card>
        <CardHeader className="pb-2"><CardTitle className="text-sm">문서 관리</CardTitle></CardHeader>
        <CardContent className="space-y-3">
