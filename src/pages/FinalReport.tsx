...
function isAtLeastFirstDone(status: string): boolean {
  return statusOrder.indexOf(status) >= statusOrder.indexOf("1차 점검완료");
}

/* Photo page slots matching Word template */
const PHOTO_SLOTS = [
  { key: "replacement_parts", title: "교체 부품 사진" },
  { key: "body_optics", title: "본체, 광학 (렌즈) 관련 부품 점검 사진" },
  { key: "cpu_smps", title: "Main Control CPU Board, SMPS, 기타 부품 점검 사진" },
  { key: "ao_probe", title: "프로브 점검 사진" },
...
export default function FinalReport() {
  ...
  // Find existing final report for selected equipment
  ...
  // Find matching 1차 report for prefill
  ...
  const equipmentWithReports = useMemo(() => {
    ...
    return new Set(
      reports
        .filter(r => r.inspection_id === selectedInspectionId && r.report_type === "final")
        .map(r => r.equipment_item_id)
    );
  ...
  const completedFinalReports = useMemo(() => {
    return reports
      .filter(r => r.report_type === "final" && r.status !== "draft")
      .sort((a, b) => b.created_date.localeCompare(a.created_date));
  }, [reports]);

  const filteredCompletedReports = useMemo(() => {
    let list = completedFinalReports;
    if (qaFilter !== "all") {
  ...
  }, [completedFinalReports, reportSearchKeyword, qaFilter, inspections]);
  ...
  const handleComplete = () => {
    ...
    toast.success("완료 점검보고서가 완료되었습니다. 품질본부에 알림이 전송됩니다.");
  };
      ...
      <div className="rounded-lg border bg-card p-4 space-y-3">
        ...
        {filteredCompletedReports.length === 0 ? (
          <p className="text-xs text-muted-foreground py-4 text-center">
            {reportSearchKeyword.trim() ? "검색 결과가 없습니다." : "완료된 완료 점검보고서가 없습니다."}
          </p>
        ) : (
        ...
        <Dialog open={!!detailReportId} onOpenChange={(open) => { if (!open) setDetailReportId(null); }}>
          <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col p-0 overflow-hidden">
            <DialogHeader className="p-6 pb-0 shrink-0 flex flex-row items-center gap-2">
              ...
              <DialogTitle className="!mt-0">완료 점검보고서 상세</DialogTitle>
            </DialogHeader>
            <DocumentView
...
/* ══════════════════════════════════════════════════════════════
   Document Form (create new final report with 1차 prefill)
   ══════════════════════════════════════════════════════════════ */
function DocumentForm({
  inspection, equipment, firstReport, inspectorName, onSubmit,
}: {
  inspection: OutboundInspection;
  equipment: OutboundEquipmentItem;
  firstReport: InspectionReport | null;
  inspectorName: string;
  onSubmit: (data: Omit<InspectionReport, "id" | "created_at" | "updated_at" | "completed_at" | "approved_at" | "approved_by">) => void;
}) {
  const today = new Date().toISOString().split("T")[0];
  const firstData = firstReport?.inspection_data;

  // Prefill serial from 1차 report or equipment
  const [serialNo, setSerialNo] = useState(
    firstReport?.serial_numbers[equipment.id] || equipment.serial_no || ""
  );

  // Prefill all data from 1차 report if available
  const [data, setData] = useState<InspectionReportData>(() => {
    if (firstData) {
      return {
        ...JSON.parse(JSON.stringify(firstData)),
        // Reset photos array (deep copy already done)
      };
    }
    ...
    <div className="p-6 pt-2 space-y-6 overflow-y-auto flex-1">
      {firstReport && (
        <div className="rounded-md bg-muted/50 border border-border px-3 py-2 text-xs text-muted-foreground">
          ℹ️ 1차 점검보고서 데이터가 자동으로 불러와졌습니다. 수정 가능합니다.
        </div>
      )}
      <TemplateBody
...
function DocumentView({
  ...
  const handleWordDownload = async () => {
    try {
      ...
      await exportReportToWord(inspection, updatedReport, "완료 점검보고서");
    ...
    <div className="flex flex-col flex-1 min-h-0">
      ...
      <div className="flex-1 overflow-y-auto pb-20 space-y-4 p-6 pt-2">
        <div className="flex items-center justify-between rounded-lg border bg-card p-3">
          <span className="text-sm font-medium">완료 점검보고서</span>
          <Badge className={cn("text-[10px]",
  ...
  return (
    ...
    <div className="space-y-8">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-lg font-bold">[완료 점검 보고서]</p>
      </div>

      {/* Report type checkbox — 완료 checked */}
      ...
      {/* Ⅱ. 점검 내용 및 조치 사항 — 4th column = "조치결과" for 완료 */}
      <div>
      ...
      <div>
        ...
        <table className="w-full border-collapse">
          ...
          <tbody>
            {(() => {
              const defaults = ["최종 점검 결과 요약", "분광기 얼라인 확인", "프로브 얼라인먼트 확인", "표준가스 교정"];
              const total = Math.max((data.summary_labels?.length || 0), data.summary_items.length, defaults.length);
              const rows = [];
                    ...
                    <td className={cn(thCls, "w-1/3")}>
                      {ro ? (
                        ...
                        <Input className="h-7 text-xs border-0 border-b rounded-none px-0 focus-visible:ring-0 bg-transparent font-semibold" value={currentLabel} placeholder="항목명" onChange={(e) => {
                          ...
                          while (arr.length <= idx) arr.push(arr.length < defaults.length ? defaults[arr.length] : "");
      ...
      {/* ═══ Photo Sections (2-column layout) ═══ */}
